...
public enum Theme {
    ...
    public static int getGradient(int index, double delay) {
        if (index > 0) {
            return convert(values()[index].firstGradient, values()[index].secondGradient, (Math.sin(System.currentTimeMillis() / 1.0E8 * 0.5 * 400000.0 + delay * 0.55) + 1.0) * 0.5).getRGB();
        }
        else if (index == 0) {
    ...
    public static int[] getGradients(int index) {
        ...
        if (values != null && index >= 0 && index < values.length && values[index] != null) {
            ...
            if (firstGradient != null && secondGradient != null) {
                ...
                return new int[]{ColorUtil.rainbow(0).getRGB(), ColorUtil.rainbow(0).getRGB()};
            }
        }
        return new int[]{0, 0};
    }
